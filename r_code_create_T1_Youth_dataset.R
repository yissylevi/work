...
most <- read.csv("Psychometic_surveys_youth_only.csv",header=T,sep=",")
Perma <- read.csv("PERMA_Psychometic_youth_only.csv",header=T,sep=",")
SDQIII <- read.csv("SDQIII_Surveys.csv",header=T,sep=",")
SDQIIP <- read.csv("SDQIIP_Surveys.csv",header=T,sep=",")
GRIT <- read.csv("GRIT_Psychometic_youth_only.csv",header=T,sep=",")
...
Perma<-select(Perma, Dataset, Gender, Age, Grade, PERMA_1  , PERMA_2  , PERMA_3  , PERMA_4  , PERMA_5  , PERMA_6  , PERMA_7  , PERMA_8
              , PERMA_9  , PERMA_10  , PERMA_11  , PERMA_12  , PERMA_13  , PERMA_14  , PERMA_15  , PERMA_16  ,
              PERMA_17)
Perma <- Perma[grep("AST1|PSQUAL", Perma$Dataset),]

most<-select(most, Dataset, Gender, Age, Grade, ASDQII_1  , ASDQII_2  , ASDQII_3  , ASDQII_4  , ASDQII_5  , ASDQII_6  , ASDQII_7  ,
...
most <- most[grep("ASCGW1|PSQUAL|ASCGW1 |SBW1 |PSF |DJT1 |Mariah |RH1 |EG", most$Dataset),]
SDQIII<-select(SDQIII, Dataset, Gender, Age, Grade, SDQIII_1, SDQIII_2, SDQIII_3, SDQIII_4, SDQIII_5, SDQIII_6, SDQIII_7,
               SDQIII_8, SDQIII_9, SDQIII_10)

SDQIIP<-select(SDQIIP, Dataset, Gender, Age, Grade, SDQIIP_1  , SDQIIP_2  , SDQIIP_3  , SDQIIP_4  , SDQIIP_5  , SDQIIP_6, SDQIIP_7  ,
               SDQIIP_8)
GRIT<-select(GRIT, Dataset, Gender, Age, Grade, GRIT_4  , GRIT_5  , GRIT_6  , GRIT_7  , GRIT_8  , GRIT_9  , GRIT_10  , GRIT_11)
GRIT <- GRIT[grep("PSQUAL", GRIT$Dataset),]
BIG5<-select(BIG5, Dataset, Gender, Age, Grade, B5F_1  , B5F_2  , B5F_3  , B5F_4  , B5F_5  , B5F_6  , B5F_7  , B5F_8  , B5F_9
...
allsurveysYT1<-cbindPad(Perma, most, SDQIII, SDQIIP, GRIT, BIG5) 
#The first row is text and we need to get rid of it
allsurveysYT1<- as.data.frame(allsurveysYT1[-1,])
#Turn into managable table
allsurveysYT1<-tbl_df(allsurveysYT1)
allsurveysYT1
...
allsurveysYT1<- data.frame(apply(allsurveysYT1,2, as.numeric))
#Now check to see if all numeric 
str(allsurveysYT1)
#Descriptive statistics
summary(allsurveysYT1)
write.csv(allsurveysYT1, file="allsurveysYT1.csv")
allsurveysYT1<-(allsurveysYT1)
##load packages
library(psych)
library(GPArotation)
library(missMDA)
