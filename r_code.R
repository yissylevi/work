...
allsurveys<-cbindPad(Perma, most, SDQIII, SDQIIP, GRIT, BIG5, CPSn) 
#The first row is text and we need to get rid of it
allsurveys<- as.data.frame(allsurveys[-1,])
#Turn into managable table
allsurveys<-tbl_df(allsurveys)
allsurveys
...
allsurveys<- data.frame(apply(allsurveys,2, as.numeric))
#Now check to see if all numeric 
str(allsurveys)
#Descriptive statistics
summary(allsurveys)
write.csv(allsurveys, file="allsurveys.csv")
##load packages
library(psych)
library(GPArotation)
library(missMDA)
