  ...
  sidebarLayout(
    sidebarPanel(
      ...
      p("Imagine you are able to choose tokens that are worth money from a huge pile of normally distributed tokens (to learn about the normal distribution",
        ...
        "You want to maximize the amount of money you can make. Note there are also token worth negative amounts of money which when drawn will loose you money. You have three choices below. Play with the options and take note of how the plot changes to determine which numbers to change to maximize your earnings."),
      ...
      numericInput(inputId = "num1", label = "The Average Value You Want Your Tokens to be Worth (between $100 and $1000):", min = 100, max = 1000, value = 100, step = 5, width="130px"),
...
server <- function(input, output) {
  output$value <- renderText({
    paste("", as.character(c(input$num, input$num1,input$num2)))
  })
  output$box1<-renderPlot({
    
    ## Plot the results
    set.seed(1234)
    x<- rnorm(n=input$num, mean=input$num1, sd=input$num2)
    sdxl<-mean(x)-sd(x)
    sdxh<-mean(x)+sd(x)
    sdxl2<-mean(x)-2*sd(x)
    sdxh2<-mean(x)+2*sd(x)
    ...
    sdxall<-as.data.frame(cbind(sdxl, sdxh, sdxl2, sdxh2, sdxl3, sdxh3))
    
    flips<-as.data.frame(x)
    
    #sdx<-as.data.frame(sdx)
    flips$x<-as.numeric(flips$x)
    require(ggplot2)
    G =  ggplot(flips, aes(x=flips$x)) +
      geom_histogram(binwidth = 1, colour="black", fill=cm.colors(n=1)) +
      ggtitle("Standard Deviation Demonstration Histogram") +
      xlab("Money in Dollars (Including Negative Money Lost)") +
      ylab("Number of Times a Token Was Drawn") +
      geom_vline(aes(xintercept=mean(x, na.rm=F)),
                 color="red", linetype="solid", size=6) +
      geom_vline(aes(xintercept = as.numeric(sdxall$sdxh)),
                 color="blue", linetype="dashed", size=2) +
      geom_vline(aes(xintercept = as.numeric(sdxall$sdxl)),
                 color="blue", linetype="dashed", size=2) +
      geom_vline(aes(xintercept = as.numeric(sdxall$sdxh2)),
                 color="green", linetype="dashed", size=2) +
      geom_vline(aes(xintercept = as.numeric(sdxall$sdxl2)),
                 color="green", linetype="dashed", size=2) +
      geom_vline(aes(xintercept = as.numeric(sdxall$sdxh3)),
                 color="orange", linetype="dashed", size=2) +
      geom_vline(aes(xintercept = as.numeric(sdxall$sdxl3)),
                 color="orange", linetype="dashed", size=2)
    G
  })
}
